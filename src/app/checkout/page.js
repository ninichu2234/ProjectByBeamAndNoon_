    ...
        const { userId, guestId, loading: userLoading } = useUser(); // เพิ่ม loading จาก context
        ...
        // --- ‼️ เพิ่ม State สำหรับ Profile Data ‼️ ---
        const [profile, setProfile] = useState(null);
        const [profileLoading, setProfileLoading] = useState(true); // สถานะ Loading สำหรับ Profile
    ...
        }, []);
        ...
        // --- ‼️ [2] Effect โหลด Profile Data (สำคัญมากสำหรับสมาชิก) ‼️ ---
        useEffect(() => {
            if (!userLoading) { // รอให้ UserContext โหลดเสร็จก่อน
                if (userId) { // ถ้าเป็นสมาชิก
                    const fetchProfile = async () => {
                        ...
                        try {
                            ...
                            if (error && error.code !== 'PGRST116') { // PGRST116 คือ No rows found
                                ...
                                // ไม่ throw error แต่ setProfileLoading เป็น false เพื่อให้หน้า Render ต่อไปได้
                            }

                            // ถ้าไม่พบข้อมูล หรือมีข้อมูล
                            setProfile(data);
                    ...
                    // ถ้าเป็น Guest (ไม่มี userId) ให้ตั้งค่า Loading เป็น false ทันที
                    setProfile(null);
                    setProfileLoading(false);
                    console.log("CheckoutPage: Not a member, skipping profile fetch.");
                }
            }
        }, [userLoading, userId]); // รันเมื่อ userLoading หรือ userId เปลี่ยน
    ...
            if (userLoading || profileLoading) { // เช็ค Profile Loading ด้วย
    ...
        // --- ส่วนแสดงผล Loading (เพิ่ม: รอ UserContext และ Profile โหลดด้วย) ---
         // ‼️ 4. แก้ไขเงื่อนไข Loading ‼️
        if (isInitialMount.current || userLoading || profileLoading) { // รอทั้งตะกร้า, User และ Profile โหลดเสร็จ
            return (
    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-[#4A3728] text-center mb-8">ข้อมูลการชำระเงิน</h1>
                    {userId && profile && (
                        <div className="max-w-6xl mx-auto p-4 mb-8 bg-amber-100 border border-amber-300 rounded-xl text-amber-900 shadow-sm">
                            <h3 className="font-semibold text-lg">สวัสดีครับ, {profile.username || profile.email || 'สมาชิก'}</h3>
                            <p className="text-sm">คุณมีแต้มสะสม: <span className="font-bold text-amber-700">{profile.points || 0} Points</span></p>
                        </div>
                    )}
    ...
                                        disabled={isProcessing || !Array.isArray(cartItems) || cartItems.length === 0 || isInitialMount.current || userLoading || profileLoading} // เพิ่ม profileLoading
